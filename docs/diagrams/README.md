...
- **Level 1 — System Context.** Aperitivo as a black box, surrounded by users and external systems (Strava, Keycloak, etc.). [c4/level-1-system-context.md](c4/level-1-system-context.md) — *to be written*
- **Level 2 — Containers.** Internal high-level deployment units (modular monolith app, Postgres+TimescaleDB, Kafka, Redis, Keycloak). [c4/level-2-containers.md](c4/level-2-containers.md) — *to be written*
...
- `sequence/strava-oauth-login.md` — *to be written*
- `sequence/activity-ingestion-webhook.md` — *to be written*
- `sequence/token-refresh.md` — *to be written*
- `sequence/initial-backfill.md` — *to be written*
- `sequence/revocation-flow.md` — *to be written*

## ERDs

Per-BC entity-relationship diagrams co-located with each context's documentation, or in `erd/` if cross-BC:

- `erd/iam.md` — Identity & Access entities — *to be written*
- `erd/catalog.md` — Workout Catalog entities — *to be written*
- (etc.)

## Conventions

- Use `flowchart` for system topology, `sequenceDiagram` for flows, `erDiagram` for ERDs.
- Name participants in sequence diagrams consistently with the BC catalog.
