...
- **No rate-limit-budget table.** Budget tracking is Redis counters per token window (see [strava-rate-limits.md]), not relational state — it is high-churn, ephemeral, and never needs to be transactional with activity data.
